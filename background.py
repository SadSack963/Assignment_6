...
def draw_gradient(start_color, target_color):
    ...
    tim.penup()
    tim.goto(-WIDTH / 2, HEIGHT / 2)  # Move to top left of screen
    tim.pendown()
    ...
    delta_color_list = [(target_component - start_color[index]) / HEIGHT for index, target_component in enumerate(target_color)]

    direction = 1  # Turtle direction of travel (left to right)
    for line, y_coord in enumerate(range(HEIGHT // 2, -HEIGHT // 2, -1)):
        tim.forward(WIDTH * direction)  # Draw a line
